...
fprintf('  Closed-loop poles: '); fprintf('% .4f  ', eig(ctrl.A - ctrl.B*ctrl.K(1:4))'); fprintf('\n\n');
...
try
    ...
    warning('dlqr on identified model failed (%s). Using bootstrap gains.', e_lqr.message);
    K_bb = ctrl.K(1:4);
end
...
try
    ...
    warning('dlqe on identified model failed (%s). Using bootstrap observer.', e_kf.message);
    L_bb = ctrl.L;
end
