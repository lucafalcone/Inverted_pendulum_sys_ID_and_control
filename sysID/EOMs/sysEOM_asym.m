function [dx, y] = sysEOM_asym(~, x, u, M, m, b, c, l, k_pos, k_neg, d_pos, d_neg, Fc, Ft, varargin)
% x = [x, v, \theta, \omega]
% y = [x, \theta]
...
% Ft is the constant force applied to the system due to a tilt in the
% system that is not perfectly parallel to the ground
...
num1 = F + Ft - c*x(2) - Fc*tanh(eps_c*x(2)) - m*l*x(4)^2*sin(x(3)) ...
